...
resource "openstack_compute_floatingip_associate_v2" "ubuntu_fip" {
    ...
    provisioner "local-exec" {
        command =  <<EOF
sudo ansible-playbook  -u deployadm -b -i '${self.floating_ip},' --private-key openstack_compute_keypair_v2.localkey.name --ssh-extra-args='-o StrictHostKeyChecking=no' playbook.yml
EOF
    }
}
